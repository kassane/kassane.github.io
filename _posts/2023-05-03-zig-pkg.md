...
- [Unofficial Release-notes - by Marler]
...
```zig
   ...
   // your executable config
    ...
    exe.addModule(mylib.module("mylib")); // <== for zig project
...
// (mylib_dep.artifact(...) == mylib): std.Build.CompileStep (eql to addSharedLib/addStaticLib/addExecutable/addObject)
...
- [libmingw32-extended](https://github.com/kassane/libmingw32-extended) - need `winpthreads` to build.


#### Modules

**Note:** No need `build.zig.zon` file to work!

```zig
exe.addModule(mylib.module("mylib")); // <== for zig project only
```
need on mylib `build.zig` add:
```zig
...
[Unofficial Release-notes - by Marler]: https://marler8997.github.io/zig-unofficial-releases/
